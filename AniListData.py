        ...
        try:
            ...
            if response_data is None or 'data' not in response_data:
                ...
                data['animeStats'] = {
                    ...
                    'episodesWatched': 
                        response_data['data']['User']['statistics']['anime']['episodesWatched'],
                    'minutesWatched': 
                        response_data['data']['User']['statistics']['anime']['minutesWatched'],
                    'meanScore': 
                        response_data['data']['User']['statistics']['anime']['meanScore'],
                    'standardDeviation': 
                        response_data['data']['User']['statistics']['anime']['standardDeviation'],
                }
                ...
                if 'animeTags' in keys:
                    ...
                    top_tags = sorted(tags, key=lambda x: x['count'],
                                      reverse=True)[:5]
                    top_tags = [{'tag': tag['tag']['name'],
                                 'count': tag['count']} for tag in top_tags]
                    data['animeTags'] = top_tags

                if 'animeVoiceActors' in keys:
                    voice_actors = response_data['data']['User']['statistics']['anime']['voiceActors']
                    top_voice_actors = sorted(voice_actors, key=lambda x: x['count'],
                                              reverse=True)[:5]
                    top_voice_actors = [{'voiceActor': actor['voiceActor']['name']['full'],
                                         'count': actor['count']} for actor in top_voice_actors]
                    data['animeVoiceActors'] = top_voice_actors

                if 'animeStudios' in keys:
                    studios = response_data['data']['User']['statistics']['anime']['studios']
                    top_studios = sorted(studios, key=lambda x: x['count'],
                                         reverse=True)[:5]
                ...
                data['mangaStats'] = {
                    ...
                    'chaptersRead': 
                        response_data['data']['User']['statistics']['manga']['chaptersRead'],
                    'volumesRead': 
                        response_data['data']['User']['statistics']['manga']['volumesRead'],
                    'meanScore': 
                        response_data['data']['User']['statistics']['manga']['meanScore'],
                    'standardDeviation': 
                        response_data['data']['User']['statistics']['manga']['standardDeviation'],
                }
                if 'mangaGenres' in keys:
                    genres = response_data['data']['User']['statistics']['manga']['genres']
                    top_genres = sorted(genres, key=lambda x: x['count'],
                                        reverse=True)[:5]
                    top_genres = [{'genre': genre['genre'],
                                   'count': genre['count']} for genre in top_genres]
                ...
                data['socialStats'] = {
                    'totalFollowers':
                        response_data['data']['followersPage']['pageInfo']['total'],
                    'totalFollowing':
                        response_data['data']['followingPage']['pageInfo']['total'],
                    'totalActivity':
            ...
            if error.response and error.response.status_code == 429:
                log_message(f'Rate limit exceeded', 'error')
                return None
            else:
                raise error
