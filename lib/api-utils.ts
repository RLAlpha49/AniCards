...
/**
 * Validate the runtime configuration that AniCards requires in production.
 *
 * This helper stays side-effect free so CI and deployment workflows can
 * preflight the contract without initializing network clients or touching the
 * request path. Callers can validate the full contract or a named subset.
 */
...
function scheduleTelemetryTask(
  ...
  if (waitUntil) {
    ...
    const pendingTask = createDeferredTelemetryTask(async () => {
      if (!shouldRunDeferredTask) return;
