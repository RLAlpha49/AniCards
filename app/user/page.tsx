...
export default async function UserPage(props: {
  ...
  try {
    ...
    if (params.cards && (params.userId || params.username)) {
      ...
      const [userRes, cardsRes] = await Promise.all([
        fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/user?userId=${params.userId}`,
        ),
        fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/cards?userId=${params.userId}`,
        ),
      ]);
      userData = await userRes.json();
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {cardTypes.length > 0 && (
        <>
          <Alert className="mx-auto mb-6 max-w-xl border-blue-500">
          ...
          <Alert className="mx-auto mb-6 max-w-xl border-green-500">
            <Info className="h-4 w-4 text-green-500" />
            <AlertTitle className="text-green-500">Spread the Word</AlertTitle>
            <AlertDescription className="text-foreground">
              If you found this tool useful, please consider crediting in your
              bio with{" "}
              <a
