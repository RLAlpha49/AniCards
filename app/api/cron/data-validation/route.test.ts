...
describe("Data Validation Cron API POST Endpoint", () => {
	...
	it("should generate a data validation report successfully with no issues", async () => {
		...
		expect(report.details["user:*"]).toEqual({ checked: 1, inconsistencies: 0 });
		expect(report.details["cards:*"]).toEqual({ checked: 1, inconsistencies: 0 });
		expect(report.details["username:*"]).toEqual({ checked: 1, inconsistencies: 0 });
