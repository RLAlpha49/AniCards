...
export async function POST(request: Request) {
  ...
  try {
    ...
    operationName =
      query.match(/(query|mutation)\s+(\w+)/)?.[2] || "anonymous_operation";
    ...
    if (operationName === "GetUserId") {
      ...
      userIdentifier = variables?.userId
        ? String(variables.userId)
        : "no_userid";
    }

    console.log(
      `🚀 [AniList API] Anilist request: ${operationName} for ${userIdentifier}`,
    );
    ...
    if (!response.ok) {
      ...
      analyticsClient
        .incr("analytics:anilist_api:failed_requests")
        .catch(() => {});
      ...
      if (response.status === 429) {
        throw new Error(
          `HTTP error! status: ${response.status} - ${errorMessage} - Rate limited${retryAfterMsg}`,
        );
      } else if (response.status === 500) {
        throw new Error(
          `HTTP error! status: ${response.status} - ${errorMessage} - Server error${retryAfterMsg}`,
        );
      } else {
        throw new Error(
    ...
    console.log(
      `✅ [AniList API] Anilist operation ${operationName} completed successfully.`,
    );
    const analyticsClient = Redis.fromEnv();
    analyticsClient
      .incr("analytics:anilist_api:successful_requests")
      .catch(() => {});
