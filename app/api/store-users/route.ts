...
export async function POST(request: Request) {
  ...
  if (!success) {
    ...
    analyticsClient
      .incr("analytics:store_users:failed_requests")
      .catch(() => {});
  ...
  try {
    ...
    if (storedRecordRaw) {
      console.log(
        `🔍 [Store Users] Found existing record for user ${data.userId}`,
      );
      try {
    ...
    console.log(
      `📝 [Store Users] Saving user data to Redis under key: ${userKey}`,
    );
    ...
    if (data.username) {
      ...
      console.log(
        `📝 [Store Users] Updating username index for: ${normalizedUsername}`,
      );
    ...
    console.log(
      `✅ [Store Users] Successfully stored user ${data.userId} [${duration}ms]`,
    );

    const analyticsClient = Redis.fromEnv();
    analyticsClient
      .incr("analytics:store_users:successful_requests")
      .catch(() => {});
    ...
    const duration = Date.now() - startTime;
    console.error(
      `🔥 [Store Users] Error after ${duration}ms: ${error.message}`,
    );
    if (error.stack) {
      console.error(`💥 [Store Users] Stack Trace: ${error.stack}`);
    }
