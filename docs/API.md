...
5. **Persist the user snapshot** — `POST /api/store-users` (`storeUser`) consumes the request-proof cookie, same-origin and verified-IP checks, and the stats-bound grant. When a stored user already exists, send **at least one** compare token: `ifMatchUpdatedAt` is the browser-facing compatibility token, while `ifMatchRevision` and `ifMatchSnapshotToken` provide stronger snapshot-bound compares. Success returns new user `updatedAt`, `revision`, and `snapshotToken`, then refreshes the lighter `stored_user` grant.
...
- Once a stored record already exists, `/api/store-users` requires **at least one** compare token. `ifMatchUpdatedAt` is the browser-facing compatibility token; `ifMatchRevision` and `ifMatchSnapshotToken` are stronger snapshot-bound alternatives. `/api/store-cards` still expects the latest cards `ifMatchUpdatedAt`, and can also use the user snapshot tokens to pin the save to a specific stored user snapshot.
